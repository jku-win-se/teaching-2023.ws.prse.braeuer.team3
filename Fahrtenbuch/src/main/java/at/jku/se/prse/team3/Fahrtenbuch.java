...
public class Fahrtenbuch {
    ...
    public List<Fahrt> filterByKategorie(String kategorie) {
        if (kategorie == null) {
            return this.fahrten;
        }
        //Filtere die Fahrten, von denen zumindest ein Kategorie in der "kategorien" liste vorkommt
        /*return this.fahrten.stream().filter(f ->
            f.getKategorien().stream().filter(k -> kategorien.contains(k)).count() > 0*/
        return this.fahrten.stream().filter(f ->
            f.getKategorien().contains(kategorie))
                .collect(Collectors.toList());
    }
}
